...
REM UltraVideoPeak Windows helper
...
if "%INPUT%"=="" (
  echo Usage: peak_video.bat input.mp4 [options]
  echo.
  echo Options:
  echo   --out FILE
  echo   --fps N
...
if %MAX%==1 (
  ...
  set PRESET=veryslow
  set FPS=120
  echo Absolute Peak Mode enabled
)

if "%OUTPUT%"=="" (
  for %%F in ("%INPUT%") do set OUTPUT=%%~nF_UltraPeak_%WIDTH%x%HEIGHT%_%FPS%fps.mp4
)
...
echo.
echo ========================================
echo   UltraVideoPeak v%VERSION% (Windows)
