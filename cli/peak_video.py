#!/usr/bin/env python3
"""
UltraVideoPeak - Python cross-platform version
...
def print_banner():
    print("╔══════════════════════════════════════════════════════╗")
    print(f"║           UltraVideoPeak v{VERSION} (Python)             ║")
...
def main():
    ...
    if args.max:
        ...
        args.fps = 120
        print("★ Absolute Peak Mode enabled")

    if not args.out:
        stem = Path(args.input).stem
        args.out = f"{stem}_UltraPeak_{args.width}x{args.height}_{args.fps}fps.mp4"
