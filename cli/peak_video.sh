#!/usr/bin/env bash
# UltraVideoPeak - Push video quality to absolute peak
...
Options:
  -o, --out FILE       Output file (default: input_peak.mp4)
...
if [[ $MAX_MODE -eq 1 ]]; then
  ...
  PRESET="veryslow"
  TARGET_FPS=120
  echo -e "${GREEN}★ Absolute Peak Mode enabled (CRF $CRF, $PRESET, ${TARGET_FPS}fps)${NC}"
fi

if [[ -z "$OUTPUT" ]]; then
  BASE=$(basename "$INPUT")
  NAME="${BASE%.*}"
  OUTPUT="${NAME}_UltraPeak_${TARGET_WIDTH}x${TARGET_HEIGHT}_${TARGET_FPS}fps.mp4"
fi
...
# Build filter
# minterpolate is heavy; use it only when FPS is increased
VF="scale=${TARGET_WIDTH}:${TARGET_HEIGHT}:flags=${SCALE_FLAGS}"

if [[ $TARGET_FPS -gt 30 ]]; then
  # mi_mode=mci is higher quality but slower
  VF="${VF},minterpolate=fps=${TARGET_FPS}:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1"
fi

# Prefer libx265 for better quality at high res, fallback to libx264
ENCODER="libx264"
