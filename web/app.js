...
startBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (parseInt(fps) > 30) {
      // Simplified minterpolate for wasm (full mci can be too heavy)
    ...
    await ffmpeg.exec([
      ...
      "-vf", vf,
      "-c:v", "libx264",
      "-preset", "medium",          // veryslow is too slow in wasm
      "-crf", crf,
      "-c:a", "aac",
      "-b:a", "192k",
      "-movflags", "+faststart",
    ...
    downloadLink.download = selectedFile.name.replace(/\.[^.]+$/, "") + `_UltraPeak_${w}x${h}_${fps}fps.mp4`;
    result.classList.remove("hidden");
    status.textContent = "Done! Download your peak quality video.";
